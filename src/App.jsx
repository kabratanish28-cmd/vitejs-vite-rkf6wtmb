...
const SUPABASE_KEY =
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImFydWVteXdqamZucHBscXBieXdlIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzI5MzM1ODUsImV4cCI6MjA4ODUwOTU4NX0.rnQ_pLhQPvtuJa9mre9_t_igopql-nf2CuTCgCP-EW4';
...
export default function App() {
  ...
  const login = async (username, password) => {
    ...
    if (data.status === 'rejected')
      return { ok: false, msg: 'Your account was rejected by the admin' };
    setUser(data);
    setScreen('market');
    return { ok: true };
  };
  ...
  const register = async (username, password, displayName) => {
    ...
    const { error } = await sb().from('mm_users').insert({
      username,
      password,
    ...
    await sb()
      .channel('user-bc')
      .send({ type: 'broadcast', event: 'user_update', payload: {} });
    return { ok: true };
  };
  ...
  return (
    <div className="login-bg">
      <div
        ...
        style={{ width: 400, border: '1px solid #2a2a44' }}
      >
        {done ? (
          <div style={{ textAlign: 'center', padding: '20px 0' }}>
            ...
            <div style={{ color: '#666', fontSize: 13, marginTop: 8 }}>
              Waiting for admin approval. Check back soon.
            </div>
            <button
              ...
              {['login', 'register'].map((m) => (
                <button
                  ...
                  style={{
                    ...
                    fontSize: 13,
                    borderRadius: 8,
                    background: mode === m ? 'rgba(99,102,241,.2)' : 'none',
        ...
        {items.map((s, i) => {
          ...
          return (
            <div key={i} className="ticker-item">
              ...
              <span className="mono" style={{ color: clr(p, pv) }}>
                {fmt(p)}
              </span>
              <span style={{ color: clr(p, pv), fontSize: 10 }}>
                {fmtPct(p, pv)}
              </span>
            </div>
          );
...
function MarketScreen({
  ...
  const stockPnl = Object.entries(portfolio).reduce(
    (a, [sym, pos]) =>
      a + ((prices[sym] ?? BASE_PRICES[sym]) - pos.avg_price) * pos.qty,
    0
  );
  const optPnl = optPort.reduce(
    (a, pos) =>
      a +
      (bsPrice(pos.type, indexPrice, pos.strike) - pos.premium) * 75 * pos.lots,
    0
  );
  const totalPnl = stockPnl + optPnl;
  const totalInvested = Object.entries(portfolio).reduce(
    (a, [sym, pos]) => a + pos.avg_price * pos.qty,
    0
  );
  const tabs = [
    'market',
    ...
    <div>
      ...
      >
        ...
        <div>
          ...
          <span
            className="mono"
            style={{ color: clrV(totalPnl), fontWeight: 700 }}
          >
            {totalPnl >= 0 ? '+' : ''}
            {fmt(totalPnl)}
          </span>
      ...
      <div style={{ padding: 24 }}>
        ...
        {tab === 'leaderboard' && (
          <LeaderboardTab user={user} prices={prices} indexPrice={indexPrice} />
        )}
        {tab === 'tradelog' && (
          <TradeLogTab trades={trades} userId={user?.id} />
        )}
      </div>
    </div>
  );
...
function MarketTab({
  ...
  const executeTrade = async (sym, type, quantity, price) => {
    ...
    if (type === 'buy') {
      ...
      newPort[sym] = {
        qty: nq,
        avg_price: (pos.avg_price * pos.qty + price * quantity) / nq,
      };
      newCash -= cost;
    } else {
      const nq = pos.qty - quantity;
      if (nq === 0) delete newPort[sym];
      else newPort[sym] = { ...pos, qty: nq };
      newCash += cost;
    }
    ...
    const newIdx =
      (Object.values(newPrices).reduce((a, v) => a + v, 0) / STOCKS.length) *
      3.1;
    await Promise.all([
      sb()
        .from('mm_users')
        .update({ cash: newCash, portfolio: newPort })
        .eq('id', user.id),
      sb().from('mm_prices').upsert({
        id: 1,
        prices: newPrices,
        index_price: newIdx,
        updated_at: new Date().toISOString(),
      }),
      sb().from('mm_trades').insert({
        user_id: user.id,
    ...
    await sb()
      ...
      .send({
        type: 'broadcast',
        event: 'px',
        payload: { prev: prices, prices: newPrices, idx: newIdx },
      });
    await sb()
      .channel('trade-bc')
      .send({
    ...
    showToast(
      `${type === 'buy' ? 'Bought' : 'Sold'} ${quantity} × ${sym} @ ${fmt(
        price
      )}`
    );
    await refreshUser();
            ...
            return (
              <div
                ...
                style={{
                  background: '#0e0e1c',
                  border: `1px solid ${
                    sel?.symbol === s.symbol ? '#6366f1' : '#1c1c30'
                  }`,
                  borderRadius: 14,
                  padding: 16,
                  cursor: 'pointer',
              ...
              >
                ...
                >
                  ...
                  <span className={`tag tag-${p >= pv ? 'green' : 'red'}`}>
                    {fmtPct(p, pv)}
                  </span>
                </div>
                <div
                  className="mono"
                  style={{ fontSize: 20, fontWeight: 700, color: clr(p, pv) }}
                >
                  {fmt(p)}
                </div>
                {pos && (
                  <div style={{ marginTop: 6, fontSize: 11, color: '#555' }}>
                    Holdings: {pos.qty} shares
                  </div>
                )}
              </div>
            );
            ...
            <>
              ...
              <div
                ...
                style={{
                  ...
                  fontWeight: 700,
                  color: clr(
                    prices[sel.symbol] ?? sel.base,
                    prevPrices[sel.symbol] ?? sel.base
                  ),
                  marginBottom: 16,
                }}
              >
                {fmt(prices[sel.symbol] ?? sel.base)}
              </div>
              <div
                style={{
                  display: 'flex',
              ...
              >
                ...
                >
                  ...
                  <span className="mono" style={{ fontWeight: 700 }}>
                    {fmt((prices[sel.symbol] ?? sel.base) * qty)}
                  </span>
                </div>
              </div>
              {mode === 'buy' ? (
...
function PortfolioTab({ portfolio, optPort, prices, indexPrice, cash }) {
  ...
  const invested = entries.reduce(
    (a, [sym, pos]) => a + pos.avg_price * pos.qty,
    0
  );
  const curVal = entries.reduce(
    (a, [sym, pos]) => a + (prices[sym] ?? BASE_PRICES[sym]) * pos.qty,
    0
  );
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div className="grid4">
        ...
        <div className="stat">
          ...
          <div className="stat-val">{fmt(invested)}</div>
        </div>
        <div className="stat">
          <div className="stat-label">Stock P&amp;L</div>
          <div className="stat-val" style={{ color: clrV(stockPnl) }}>
            {stockPnl >= 0 ? '+' : ''}
            {fmt(stockPnl)}
          </div>
        </div>
        <div className="stat">
          <div className="stat-label">Options P&amp;L</div>
          <div className="stat-val" style={{ color: clrV(optPnl) }}>
            {optPnl >= 0 ? '+' : ''}
            {fmt(optPnl)}
          </div>
        </div>
              ...
              {entries.map(([sym, pos]) => {
                ...
                return (
                  <tr key={sym}>
                    ...
                    <td
                      className="mono"
                      style={{ color: clr(cmp, pos.avg_price) }}
                    >
                      {fmt(cmp)}
                    </td>
                    <td className="mono">{fmt(cmp * pos.qty)}</td>
                    <td className="mono" style={{ color: clrV(pnl) }}>
                      {pnl >= 0 ? '+' : ''}
                      {fmt(pnl)}
                    </td>
                    <td>
                      <span className={`tag tag-${pnl >= 0 ? 'green' : 'red'}`}>
                        {fmtPct(cmp, pos.avg_price)}
                      </span>
                    </td>
                  </tr>
                );
              ...
              {optPort.map((pos, i) => {
                ...
                return (
                  <tr key={i}>
                    <td>
                      <b>
                        {INDEX_NAME} {pos.strike} {pos.type}
                      </b>
                    </td>
                    <td>
                      <span
                        className={`tag tag-${
                          pos.type === 'CE' ? 'green' : 'red'
                        }`}
                      >
                        {pos.type}
                      </span>
                    ...
                    <td
                      className="mono"
                      style={{ color: clr(cmp, pos.premium) }}
                    >
                      {fmt(cmp)}
                    </td>
                    <td className="mono" style={{ color: clrV(pnl) }}>
    ...
    if (mode === 'buy') {
      ...
      const newOptPort = [
        ...optPort,
        {
          ...
          lots,
          premium,
          bought_at: indexPrice,
        },
      ];
      await sb()
        .from('mm_users')
        .update({ options_portfolio: newOptPort, cash: cash - totalCost })
        .eq('id', user.id);
      await sb()
        .from('mm_trades')
        .insert({
          user_id: user.id,
          display_name: user.display_name,
          symbol: `${INDEX_NAME} ${sel.strike} ${sel.type}`,
          type: 'buy',
          qty: lots,
          price: premium,
          is_admin: false,
      ...
      showToast(
        `Bought ${lots} lot(s) of ${INDEX_NAME} ${sel.strike} ${
          sel.type
        } @ ${fmt(premium)}`
      );
    } else {
      const idx = optPort.findIndex(
        (p) => p.strike === sel.strike && p.type === sel.type
      );
      if (idx === -1) {
        showToast('No position found!', 'error');
        return;
      }
      const pos = optPort[idx];
      if (lots > pos.lots) {
        showToast('Not enough lots!', 'error');
        return;
      }
      const pnl = (premium - pos.premium) * 75 * lots;
      const newOptPort = [...optPort];
      if (lots === pos.lots) newOptPort.splice(idx, 1);
      else newOptPort[idx] = { ...pos, lots: pos.lots - lots };
      await sb()
        .from('mm_users')
        .update({
          options_portfolio: newOptPort,
          cash: cash + premium * 75 * lots,
        })
        .eq('id', user.id);
      showToast(
        `Sold ${lots} lot(s) — P&L: ${pnl >= 0 ? '+' : ''}${fmt(pnl)}`,
        pnl >= 0 ? 'success' : 'info'
      );
    }
    await refreshUser();
        ...
        >
          <div>
            <span style={{ fontWeight: 800, fontSize: 17 }}>
              ⚡ {INDEX_NAME} Option Chain
            </span>
            <span
              className="mono"
              style={{ marginLeft: 12, color: '#f59e0b', fontWeight: 700 }}
            >
              {fmt(indexPrice)}
            </span>
          </div>
          ...
          <table>
            <thead>
              <tr>
                ...
                <th style={{ textAlign: 'center', color: '#f59e0b' }}>
                  STRIKE
                </th>
                <th style={{ color: '#ff1744', textAlign: 'right' }}>
                  PE Premium
                </th>
              </tr>
            </thead>
            <tbody>
              {strikes.map((strike) => {
                ...
                return (
                  <tr
                    key={strike}
                    style={isAtm ? { background: 'rgba(245,158,11,.06)' } : {}}
                  >
                    <td>
                      <button
                        className="btn btn-buy btn-sm"
                        onClick={() => {
                          setSel({ strike, type: 'CE' });
                          setMode('buy');
                        }}
                      >
                        {fmt(ce)}
          ...
          {sel ? (
            <>
              <div style={{ fontWeight: 800, fontSize: 15, marginBottom: 6 }}>
                {INDEX_NAME} {sel.strike} {sel.type}
              </div>
              <span
                className={`tag tag-${sel.type === 'CE' ? 'green' : 'red'}`}
                style={{ marginBottom: 14, display: 'inline-block' }}
              >
                {sel.type === 'CE' ? 'CALL' : 'PUT'}
              </span>
              <div
                className="mono"
                style={{
              ...
              >
                {fmt(bsPrice(sel.type, indexPrice, sel.strike))}
              ...
              >
                ...
                >
                  ...
                  <span className="mono" style={{ fontWeight: 700 }}>
                    {fmt(bsPrice(sel.type, indexPrice, sel.strike) * 75 * lots)}
                  </span>
                </div>
              </div>
              <button
...
function NewsTab({ news }) {
  return (
    <div style={{ maxWidth: 700 }}>
      ...
      {news.length === 0 ? (
        <div style={{ color: '#444', textAlign: 'center', padding: 50 }}>
          No news posted yet
        </div>
      ) : (
        news.map((n, i) => (
          <div
            key={i}
            className="news-item slide-right"
            style={{
...
function LeaderboardTab({ user, prices, indexPrice }) {
  ...
  useEffect(() => {
    sb()
      ...
      .eq('status', 'approved')
      .then(({ data }) => {
        if (data) setUsers2(data);
      });
  }, []);
  const ranked = users2
    .map((u) => {
      const sv = Object.entries(u.portfolio ?? {}).reduce(
        (a, [sym, pos]) => a + (prices[sym] ?? BASE_PRICES[sym]) * pos.qty,
        0
      );
      const ov = (u.options_portfolio ?? []).reduce(
        (a, p) => a + bsPrice(p.type, indexPrice, p.strike) * 75 * p.lots,
        0
      );
            ...
            {ranked.map((u, i) => (
              ...
              >
                ...
                <td>
                  ...
                  {u.id === user?.id && (
                    <span
                      style={{ marginLeft: 6, fontSize: 10, color: '#6366f1' }}
                    >
                      (you)
                    </span>
                  )}
                ...
                <td className="mono" style={{ color: clrV(u.pnl) }}>
                  {u.pnl >= 0 ? '+' : ''}
                  {fmt(u.pnl)}
                </td>
                <td>
                  <span className={`tag tag-${u.pnl >= 0 ? 'green' : 'red'}`}>
                    {fmtPct(u.total, STARTING_BALANCE)}
                  </span>
                </td>
              </tr>
            ))}
    ...
    <div>
      ...
      >
        <div style={{ fontWeight: 700, fontSize: 16 }}>
          📋 Trade Log <span className="live-dot" style={{ marginLeft: 8 }} />
        </div>
        <div style={{ display: 'flex', gap: 6 }}>
          {['all', 'mine', 'institute'].map((f) => (
            <button
              ...
              onClick={() => setFilter(f)}
              style={{
                color: filter === f ? '#818cf8' : '#555',
                background: filter === f ? 'rgba(99,102,241,.1)' : undefined,
              }}
            >
        ...
        <table>
          ...
          <tbody>
            {filtered.slice(0, 80).map((t, i) => (
              <tr key={i}>
                <td style={{ color: '#444', fontSize: 11 }} className="mono">
                  {tsStr(t.created_at)}
                </td>
                <td>
                  <b style={{ color: t.is_admin ? '#f59e0b' : '#e2e2f0' }}>
                    {t.is_admin ? '🏦 Institute' : t.display_name}
                  </b>
                </td>
                <td>
                  <b>{t.symbol}</b>
                ...
                <td>
                  <span
                    className={`tag tag-${t.type === 'buy' ? 'green' : 'red'}`}
                  >
                    {t.type?.toUpperCase()}
                  </span>
                </td>
...
function ResetButton({
  ...
  const doReset = async () => {
    ...
    try {
      ...
      await sb().from('mm_prices').upsert({
        id: 1,
        prices: BASE_PRICES,
        index_price: BASE_INDEX,
        updated_at: new Date().toISOString(),
      });
      await sb()
        .channel('price-bc')
        .send({
          type: 'broadcast',
          event: 'px',
          payload: { prev: BASE_PRICES, prices: BASE_PRICES, idx: BASE_INDEX },
        });
      ...
      {confirm && (
        <div className="modal-bg" onClick={() => setConfirm(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            >
              This will delete ALL trades, news, and user accounts, and reset
              all prices to base values.
              <br />
              <b style={{ color: '#ff1744' }}>This cannot be undone.</b>
            </div>
...
function AdminMarketTab({ prices, prevPrices, indexPrice, showToast }) {
  ...
  const executeTrade = async () => {
    ...
    showToast(
      `Institute ${mode === 'buy' ? 'bought' : 'sold'} ${qty} × ${
        sel.symbol
      } @ ${fmt(price)}`
    );
    setCustomPrice('');
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 20 }}>
      <div>
        ...
        <div style={{ color: '#555', fontSize: 13, marginBottom: 16 }}>
          Trade as "Institute" — your trades are visible to all participants and
          move prices via volume
        </div>
        <div
          style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}
        >
            ...
            return (
              <div
                ...
                style={{
                  background: '#0e0e1c',
                  border: `1px solid ${
                    sel?.symbol === s.symbol ? '#f59e0b' : '#1c1c30'
                  }`,
                  borderRadius: 14,
                  padding: 16,
                  cursor: 'pointer',
            ...
            <>
              ...
              <div style={{ marginBottom: 10 }}>
                ...
                <div style={{ fontSize: 11, color: '#444', marginTop: 4 }}>
                  Higher volume = bigger price impact
                </div>
              </div>
              <div style={{ marginBottom: 14 }}>
                ...
                <input
                  ...
                  placeholder={`Market: ${fmt(prices[sel.symbol] ?? sel.base)}`}
                />
              </div>
              <div
                style={{
            ...
            <tbody>
              {approved.map((u) => (
                <tr key={u.id}>
                  ...
                  <td className="mono" style={{ color: '#555' }}>
                    {u.username}
                  </td>
      ...
      <div className="card" style={{ marginBottom: 24 }}>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <textarea
            ...
            placeholder="Write your market news update here. This will immediately appear on all participant screens..."
          />
        </div>
        <button
          className="btn btn-primary"
          ...
          return (
            <div
              ...
              style={{
                ...
                cursor: 'pointer',
                marginBottom: 6,
                background: selId === u.id ? 'rgba(99,102,241,.1)' : '#0a0a16',
                border: `1px solid ${selId === u.id ? '#6366f1' : '#1a1a2e'}`,
                transition: 'all .2s',
              }}
            >
              ...
              >
                ...
                <span style={{ fontSize: 11, color: clrV(pnl) }}>
                  {pnl >= 0 ? '+' : ''}
                  {fmtPct(total, STARTING_BALANCE)}
                </span>
              </div>
            </div>
          );
          ...
          <>
            ...
            <div className="grid4" style={{ marginBottom: 20 }}>
              {(() => {
                const sv = Object.entries(u.portfolio ?? {}).reduce(
                  (a, [sym, pos]) =>
                    a + (prices[sym] ?? BASE_PRICES[sym]) * pos.qty,
                  0
                );
                const ov = (u.options_portfolio ?? []).reduce(
                  (a, p) =>
                    a + bsPrice(p.type, indexPrice, p.strike) * 75 * p.lots,
                  0
                );
                const total = u.cash + sv + ov;
                const pnl = total - STARTING_BALANCE;
                return (
                  <>
                    ...
                    <div className="stat">
                      ...
                      <div className="stat-val">{fmt(sv)}</div>
                    </div>
                    <div className="stat">
                    ...
                    <div className="stat">
                      ...
                      <div className="stat-val" style={{ color: clrV(pnl) }}>
                        {pnl >= 0 ? '+' : ''}
                        {fmt(pnl)}
                      </div>
                    </div>
            ...
            <div className="card" style={{ marginBottom: 16 }}>
              ...
              {Object.entries(u.portfolio ?? {}).length === 0 ? (
                <div
                  style={{ color: '#444', padding: 20, textAlign: 'center' }}
                >
                  No stock positions
                </div>
              ) : (
